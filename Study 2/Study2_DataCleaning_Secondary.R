...
data <- read.csv(
  here("Data", "Deidentified Raw Data", "Study2_Deidentified_Data.csv")
)
...
TidyData <- TidyData %>%
  ...
  select(PID, Relationship_Type, Relationship, Orientation, Emotion, Valence, Type, Frequency, Connection, everything()) %>%
  arrange(PID, Relationship_Type, Relationship, Orientation, Emotion)

# Create df to combine with study 1
Study2 <- TidyData %>%
  mutate(PE = "Perception") %>%
...
# Save data for comparison to study 1 to csv
write.csv(
  Study2,
